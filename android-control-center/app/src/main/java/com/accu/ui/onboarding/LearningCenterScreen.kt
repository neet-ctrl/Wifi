...
val guides = listOf(
    ...
    Guide(
        "Mastering Audio with Sound Master",
        "Per-app volume, EQ, balance and routing",
        listOf(
            "Go to Sound Master from the Dashboard",
            "Adjust individual app volumes beyond system limits (0–200%)",
            "Open the 3-band EQ and pick a preset or tune manually",
            "Enable Loudness Enhancer for quiet content",
            "Use the stereo balance slider for hearing comfort",
            "Set audio routing per-app (speaker, earpiece, Bluetooth)",
            "Save your full audio profile as a named preset",
        ),
        Icons.Default.VolumeUp, "Beginner",
    ),
    Guide(
        "Managing Audio Focus with Mixed Audio",
        "Control which apps play and which stay silent",
        listOf(
            "Go to Mixed Audio from the Dashboard",
            "See which app currently holds audio focus",
            "Tap any app to mute or revoke its audio focus",
            "Use batch mute to silence all background apps at once",
            "Save your mute configuration as a named profile",
            "Load a profile (e.g. 'Gaming', 'Work') with one tap",
        ),
        Icons.Default.Tune, "Beginner",
    ),
    Guide(
